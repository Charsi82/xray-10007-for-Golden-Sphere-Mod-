...
void CUITalkWnd::InitTalkDialog()
{
	...
	UITalkDialogWnd->UIToTradeButton.Show( m_pOthersInvOwner->IsTradeEnabled() ); //скрытие нопки перехода к торговле
...
bool CUITalkWnd::OnKeyboard(int dik, EUIMessages keyboard_action)
{
	...
	// переключиться на торговлю по кнопке экшена kSPRINT_TOGGLE (X)
