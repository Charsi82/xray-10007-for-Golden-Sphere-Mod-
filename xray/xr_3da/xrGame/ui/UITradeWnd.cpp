...
// 	int acc				= keyname_to_dik("kX");
// 	(&m_uidata->UIPerformTradeButton)->SetAccelerator(acc, 0);

	AttachChild							(&m_uidata->UIToTalkButton);
	xml_init.Init3tButton					(uiXml, "button", 1, &m_uidata->UIToTalkButton);
// 	acc				= keyname_to_dik("kF");
// 	(&m_uidata->UIToTalkButton)->SetAccelerator(acc, 0);
...
// 	{
// 		//SendMessage(&m_uidata->UIPerformTradeButton, );
// 		GetMessageTarget()->SendMessage(&m_uidata->UIPerformTradeButton,BUTTON_CLICKED);
// 		//PerformTrade();
// 		return true;
// 	}
// 
...
// 	{
// 		GetMessageTarget()->SendMessage(&m_uidata->UIToTalkButton, BUTTON_CLICKED);
// //		SendMessage(&m_uidata->UIToTalkButton, BUTTON_CLICKED, this);
// 		//SwitchToTalk();
// 		return true;
// 	}
...
void move_item(CUICellItem* itm, CUIDragDropListEx* from, CUIDragDropListEx* to)
{
	if(pInput->iGetAsyncKeyState(DIK_LCONTROL) )
	{
		...
		to->SetItem				(_itm);
		if (itm != _itm) move_item(itm, from, to); // забираем пачку иконок
	}
	else
	{
		CUICellItem* _itm		= from->RemoveItem	(itm, false);
