...
//. ???	m_fSatietyCritical			= pSettings->r_float(section,"satiety_critical");
	...
	m_MaxWalkWeight					= _min(80.0f, pSettings->r_float(section,"max_walk_weight"));
	//m_MaxWalkWeight					= _min(!gs_check_files()?0:80.0f, pSettings->r_float(section,"max_walk_weight"));
	//if (!gs_check_files()) object().inventory().SetMaxWeight(0);
}
