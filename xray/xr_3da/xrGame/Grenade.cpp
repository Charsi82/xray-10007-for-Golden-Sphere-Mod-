	...
	case kWPN_NEXT:
		{
			if(flags&CMD_START) 
			{
				if(m_pCurrentInventory)
				{
					...
					TIItemContainer::iterator it = m_pCurrentInventory->m_ruck.begin();
					for(;it!=m_pCurrentInventory->m_ruck.end();++it)
					{
						CGrenade *pGrenade = smart_cast<CGrenade*>(*it); 
						if (pGrenade && xr_strcmp(pGrenade->cNameSect(), cNameSect())) grens.push_back(pGrenade);	//собираем гранаты с секцией, несовпадающей текущей				
					};

					if (!grens.size()) return true; // не на что переключать
