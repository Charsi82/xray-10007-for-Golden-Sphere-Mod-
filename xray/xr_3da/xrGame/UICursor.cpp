...
void CUICursor::InitInternal()
{
	...
	sz.set						(rect.rb);
	if(UI()->is_16_9_mode())
		sz.x					/= (Device.dwWidth)/float(Device.dwHeight) / (UI_BASE_WIDTH/UI_BASE_HEIGHT +0.01f)/*1.2f*/;
...
#endif
	if (psHUD_Flags.test(HUD_CURSOR_DBG))
	{
		if (last_render_frame + 5 < Device.dwFrame)
		{
			last_render_frame = Device.dwFrame;		
			if (pInput->iGetAsyncKeyState(DIK_LALT))
				if(vAnchor.magnitude()>0.01f)
					vAnchor.set(0.f, 0.f);		
				else
					vAnchor.set(GetCursorPosition());
		}
		...
		F->SetHeightI		(0.02f);
		F->SetColor			(0xffffffff);
		F->OutSetI			(v_dbg_cursor.x, v_dbg_cursor.y);
		Fvector2			pt = GetCursorPosition();
		F->OutNext			("(%.f)-(%.f) [%.f-%.f]", pt.x - vAnchor.x, pt.y - vAnchor.y, vAnchor.x, vAnchor.y);
	}
