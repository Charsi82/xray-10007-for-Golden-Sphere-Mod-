...
extern Fvector2 v_dbg_cursor;
//#define GS_RELEASE // релизная версия для игрока
...
public:
	...
	virtual void Execute(LPCSTR args) {
		...
		#endif
#ifndef GS_RELEASE
		if(!strstr(Core.Params,"-dbg")) return;

		Console->Hide	(); // close console
		if (MainMenu()->IsActive()) MainMenu()->Activate(false); // close main menu if it is open

//		CUIDialogWnd* pDialog = HUD().GetUI()->MainInputReceiver(); // close active_window
//		if (pDialog && pDialog->IsShown()) HUD().GetUI()->StartStopMenu(pDialog, false);
...
public:
	...
	virtual void Execute(LPCSTR args) {
		...
		BOOL bVal;
		if ( Console->GetBool("g_hardsave", bVal) )
		{
			luabind::functor<bool>	functor;
			if (!ai().script_engine().functor("_g.can_save",functor))
			{
				Msg("can't find function '_g.can_save'");
				return;
			}
			if(!functor())
			{
				...
				_s->wnd()->SetText			(CStringTable().translate("st_cant_game_save_now").c_str());
				return;
			}
		}
...
struct CCC_GDM : public IConsole_Command {
	CCC_GDM(LPCSTR N) : IConsole_Command(N)  {};
	...
	virtual void Execute(LPCSTR args) {
		...
		BOOL bWhatToDo = TRUE;
		if( 0==xr_strlen(args) ){
			bWhatToDo = psActorFlags.test(AF_GODMODE);
		};
		...
		if( EQ(args,"off")||EQ(args,"0") )
			bWhatToDo = FALSE;
		psActorFlags.set(AF_GODMODE, bWhatToDo);
#endif
	};
	virtual void	Status	(TStatus& S)
	{	
		sprintf_s	(S, "%s", psActorFlags.test(AF_GODMODE)?"on":"off" );	  
	}
};
...
//#endif // MASTER_GOLD
	...
	psHUD_Flags.set(HUD_CROSSHAIR_BUILD, false);
...
//#endif // MASTER_GOLD
	...
	CMD3(CCC_Mask,		"g_hardsave",			&psActorFlags,	AF_HARDSAVE);
...
#endif
	v_dbg_cursor.set(0.f, -0.9f);
	CMD3(CCC_Mask,			"cl_dynamiccrosshair",	&psHUD_Flags,	HUD_CROSSHAIR_DYNAMIC);
#ifndef GS_RELEASE
	CMD3(CCC_Mask,			"dbg_cursor",			&psHUD_Flags,	HUD_CURSOR_DBG);
 	CMD4(CCC_Float,	"dbg_cursor_x",		&(v_dbg_cursor.x),	-1.f,	1.f);
 	CMD4(CCC_Float,	"dbg_cursor_y",		&(v_dbg_cursor.y),	-1.f,	1.f);
#endif
