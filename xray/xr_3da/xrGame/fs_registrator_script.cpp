...
bool gs_check_files()
{
	...
	auto list = file_list_open_ex(&FS, "$game_scripts$", FS_ListFiles, "*.script");
	list.Sort(FS_file_list_ex::eSortByModifDown);
	FS_item item = list.GetAt(0);
	Msg("script item = %s", item.NameFull());
// 	if (!strstr(item.NameFull(),"my_test")) 
// 	{
// 		Msg("last modif script no 'my_test' == [%s]", item.NameFull());
// 		return false;
// 	}
// 	if (item.Size()) 
// 	{
// 		Msg("'my_test' not empty size [%d]",item.Size());
// 		return false;
// 	}
	u32 check_modif = item.modif;

	list = file_list_open_ex(&FS, "$game_config$", FS_ListFiles, "*actor.ltx");
	list.Sort(FS_file_list_ex::eSortByModifDown);
	item = list.GetAt(1);
	Msg("config item = %s", item.NameFull());
	Msg("1 = %d", check_modif);
	Msg("2 = %d", item.modif);
	if (item.modif>check_modif) 
	{
		Msg("check modif failed");
		return false;
	}
