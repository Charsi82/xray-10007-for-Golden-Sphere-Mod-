...
void CApplication::Level_Set(u32 L)
{
	...
	if (Level_Current != L )
	{
		...
		strconcat(sizeof(level_folder), level_folder, "intro\\intro_", Levels[L].folder);
		u32 len = xr_strlen(level_folder);
		if (level_folder[len - 1] == '\\')
			level_folder[len - 1] = 0;

		int count = 0;
		string_path temp2;
		while (true)
		{
			sprintf_s(path,"%s%d", level_folder, count+1);
		...
		if (count)
			sprintf_s(path,"%s%d", level_folder, ::Random.randI(count) + 1);		
		else if (FS.exist(temp2, "$game_textures$", level_folder, ".dds"))
			sprintf_s(path, "%s", level_folder);		
	}

	hLevelLogo.create("font", path[0] ? path : "intro\\intro_no_start_picture");
